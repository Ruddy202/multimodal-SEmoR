...
def AttentionResNet():
    model = Sequential()
    #model.add(keras.Input(shape=(200, 189, 1)))
    model.add(Conv2D(32, (5, 5), padding='same')) 
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(MaxPool2D(pool_size=(2, 2))) # 16x16
    model.add(residual_block(input_channels=32, output_channels=128))
    model.add(attention_block(encoder_depth=2))
    model.add(residual_block(input_channels=128, output_channels=256, stride=2))  # 8x8
    model.add(attention_block(encoder_depth=1))
    model.add(residual_block(input_channels=256, output_channels=512, stride=2)) # 4x4
    model.add(attention_block(encoder_depth=1))
    model.add(residual_block(input_channels=512, output_channels=1024))
    model.add(residual_block(input_channels=1024, output_channels=1024))
    model.add(residual_block(input_channels=1024, output_channels=1024))
    model.add(AveragePooling2D(pool_size=(4, 4), strides=(1, 1)))  # 1x1
    model.add(Flatten())
